...
function isImportFromCurrentFolderIndex(contextFilename, resolvedSource) {
  const isIndex = basename(contextFilename, extname(contextFilename)) === "index";
...
module.exports = {
  ...
  create(context) {
    function checkNode(node) {
      if (!node.source || !node.source.value || !isImageImport(node.source.value)) {
        return;
      }

      const resolvedSource = resolve(node.source.value, context);

      if (resolvedSource && !isImportFromCurrentFolderIndex(context.getFilename(), resolvedSource)) {
        context.report({
          node,
