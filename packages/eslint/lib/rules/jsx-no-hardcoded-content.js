const { getImportDetailsForName, DEFAULT_IMPORT, NAMESPACE_IMPORT, docsUrl } = require("../utilities");
...
module.exports = {
  ...
  create(context) {
    ...
    const defaultOptionsForUnspecifiedDOMElement = Object.assign({}, defaultOptions, optionsForAllDOM);

    function optionsForNode(node) {
      if (isDOMElement(node)) {
        const { name } = node.openingElement.name;

        return dom[node.openingElement.name.name] || defaultDOMOptions[name] || defaultOptionsForUnspecifiedDOMElement;
      }
    ...
    return {
      JSXElement(node) {
        ...
        const elementName = context.getSourceCode().getText(node.openingElement.name);

        if (check.prop === "children") {
          context.report(node, `Do not use hardcoded content as the children of the ${elementName} component.`);
        } else if (check.prop) {
          context.report(
            node,
...
function getImportDetailsForJSX({ openingElement }, context) {
  ...
  const searchForName = isMemberExpression ? openingElement.name.object.name : openingElement.name.name;
...
function checkContent(node, { allowStrings = false, allowNumbers = true, checkProps = [] }) {
  function isInvalidContent(contentNode) {
    return (
      ((contentNode.type === "Literal" || contentNode.type === "JSXText") &&
        ((!allowStrings && typeof contentNode.value === "string" && !isEmptyString(contentNode.value)) ||
          (!allowNumbers && typeof contentNode.value === "number"))) ||
      (contentNode.type === "TemplateLiteral" && !allowStrings) ||
      (contentNode.type === "JSXExpressionContainer" && isInvalidContent(contentNode.expression))
    );
  }
  ...
  function isInvalidProp(propNode) {
    return (
      ...
      isInvalidContent(propNode.value == null ? { type: "Literal", value: true } : propNode.value)
    );
  }
  ...
  const invalidProp = node.openingElement.attributes && node.openingElement.attributes.find(isInvalidProp);
  return invalidProp ? { valid: false, prop: invalidProp.name.name } : { valid: true };
}
