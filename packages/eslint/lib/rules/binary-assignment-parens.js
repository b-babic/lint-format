...
module.exports = {
  ...
  create(context) {
    ...
    function hasParens(node) {
      ...
      const hasBeforeParen = beforeToken && beforeToken.type === "Punctuator" && beforeToken.value === "(";
      const afterToken = context.getTokenAfter(node);
      const hasAfterParen = afterToken && afterToken.type === "Punctuator" && afterToken.value === ")";
