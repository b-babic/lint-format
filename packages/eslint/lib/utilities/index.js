...
function uncast(node) {
  ...
  while (currentNode.type === "TypeCastExpression" || currentNode.type === "TSAsExpression") {
...
function normalizeSource(source, context) {
  ...
  const relativeSource = sourceRoot === packageRoot ? relative(packageRoot, source) : relative(packageRoot, sourceRoot);
  const sourceBasename = basename(relativeSource);
  const sourceDir = IS_FILE.test(relativeSource) ? dirname(relativeSource) : relativeSource;
